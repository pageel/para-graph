...
export function parsePlanSpecMapping(planFilePath: string): string[] | null {
  ...
  // Find ## CSA Spec Mapping Table heading (case-insensitive)
  let tableStartIndex = -1;
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].trim().toLowerCase().startsWith('## csa spec mapping table')) {
      tableStartIndex = i;
      break;
    }
  }
  ...
  if (tableStartIndex === -1) {
    console.warn(`[Plan Parser] Heading "## CSA Spec Mapping Table" not found in ${planFilePath}`);
    return null;
  }
